...
export const constructorReducer = (state = constructorInitialState, action) => {
  switch (action.type) {
    case INGREDIENT_ADDED: {
      ...
      const uid = item.uid || (typeof crypto !== 'undefined' && crypto.randomUUID
        ? crypto.randomUUID()
        : `${Date.now()}-${Math.random()}`)
      return {
        ...state,
        constructorIngredients: [...state.constructorIngredients, { ...item, uid }],
      }
    }
    case INGREDIENT_MOVED: {
      ...
      return {
        ...state,
        constructorIngredients: bun ? [bun, ...next] : next,
      }
    }
    default:
      return state
